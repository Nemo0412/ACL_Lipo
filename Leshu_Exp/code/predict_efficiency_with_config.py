"""
使用 Qwen/Qwen3-32B 模型预测虚拟库中每个药物的效率分数（使用配置文件）
"""
import pandas as pd
import json
...
def main():
    ...
    # 检查是否存在中间结果
    intermediate_file = '../data/efficiency_scores_intermediate.json'
    if os.path.exists(intermediate_file):
        ...
        if response.lower() == 'y':
            with open(intermediate_file, 'r', encoding='utf-8') as f:
                results = json.load(f)
            print(f"已加载 {len(results)} 条已处理的结果")
    ...
    # 对每个药物进行预测
    print(f"\n开始从第 {start_idx + 1} 个药物预测效率分数...")
    for idx in tqdm(range(start_idx, len(df)), desc="预测进度", initial=start_idx, total=len(df)):
        row = df.iloc[idx]
    ...
    for drug in results_sorted:
        score = drug['efficiency_score']
        if score <= 2:
            score_ranges["0-2分 (极低)"] += 1
        elif score <= 4:
            score_ranges["3-4分 (较低)"] += 1
        elif score <= 6:
            score_ranges["5-6分 (中等)"] += 1
        elif score <= 8:
            score_ranges["7-8分 (较高)"] += 1
        else:
            score_ranges["9-10分 (极高)"] += 1
