#!/usr/bin/env python3
"""
Use LANTERN pretrained model to evaluate test data.

Configuration: set LANTERN_REPO_PATH and DATA_DIR below to match your environment.
"""
import os
import sys
...
LANTERN_REPO_PATH = os.environ.get('LANTERN_REPO_PATH',
                                   'LANTERN_REPO_PATH')
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
DATA_DIR = os.path.join(BASE_DIR, 'data')
RESULTS_DIR = os.path.join(BASE_DIR, 'data', 'lantern_results')
# ──────────────────────────────────────────────────────────────────────────────

sys.path.insert(0, LANTERN_REPO_PATH)
...
def load_fingerprints(data_name):
    ...
    fp_dir = f'LANTERN_REPO_PATH/data/fingerprints/{data_name}'
...
def evaluate_efficiency():
    ...
    # Load data
    smiles_list, labels = load_labels('LANTERN_REPO_PATH/data/efficiency_test_qwen.csv')
    ...
    # Load scaler
    scaler = joblib.load('LANTERN_REPO_PATH/checkpoints/circular-expert-model-MLP_scaler.pkl')
    ...
    model.load_state_dict(torch.load('LANTERN_REPO_PATH/checkpoints/circular-expert-model-MLP.pth', map_location='cpu'))
    model.eval()
...
def evaluate_toxicity():
    ...
    # Load data
    smiles_list, labels = load_labels('LANTERN_REPO_PATH/data/toxicity_test_qwen.csv')
